...
import { User, UserClinic, Clinic, Role, Patient } from '../models';
    ...
    try {
      ...
      if (role === 'patient') {
        try {
          const patientData = {
            ...
            email,
            phone,
            date_of_birth: req.body.date_of_birth || new Date('2000-01-01'), // Default DOB if not provided
      ...
      if (role !== 'super_admin') {
        try {
          ...
          console.log(`Role lookup result for '${role}':`, roleDoc ? 'Found' : 'Not Found');
          ...
          if (roleDoc) {
            ...
            // For patients, create a simple UserClinic without role assignment as a fallback
            if (role === 'patient') {
              console.log(`Creating fallback UserClinic for patient without role assignment...`);
              const fallbackUserClinic = {
                tenant_id: clinic.tenant_id,
                user_id: user._id,
                clinic_id: clinic_id,
                roles: [],
              ...
              await UserClinic.create(fallbackUserClinic);
              console.log(`✅ Created fallback UserClinic for patient ${user.email}`);
              
              // Don't return error for patients, continue with registration
            } else {
              // Return error response for other roles
              res.status(500).json({
                success: false,
