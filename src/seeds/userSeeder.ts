...
import { User, UserClinic, Role, Clinic, Patient } from '../models';
...
export async function seedUsers(clinicIds: mongoose.Types.ObjectId[], tenants: any[]): Promise<void> {
  ...
  try {
    ...
    const roleTemplates = {
      ...
      patient: 3     // 3 patient users per clinic
    };
      ...
      for (const [role, count] of Object.entries(roleTemplates)) {
        for (let i = 0; i < count; i++) {
          ...
          const userData = {
            ...
            role: role as 'admin' | 'doctor' | 'nurse' | 'receptionist' | 'accountant' | 'staff' | 'patient',
            // Ensure patients are always active for demo purposes
            is_active: role === 'patient' ? true : faker.datatype.boolean({ probability: 0.95 }),
            is_verified: faker.datatype.boolean({ probability: 0.9 }),
            base_currency: 'USD',
            timezone: 'America/New_York',
            address: faker.location.streetAddress({ useFullAddress: true }),
            bio: role === 'patient' ? undefined : faker.lorem.paragraph(),
          ...
          // Create corresponding Patient record for patient role users
          if (role === 'patient') {
            try {
              const patientData = {
                tenant_id: clinic.tenant_id,
                ...
                first_name: firstName,
                last_name: lastName,
                email: email, // Same email as user for linking
              ...
              console.error(`    Error creating patient record for ${email}:`, error);
            }
          }
