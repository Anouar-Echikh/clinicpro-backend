...
router.post('/plans', [
  ...
  body('currency')
    .optional()
    .isIn(['USD', 'EUR', 'GBP', 'INR', 'CAD', 'AUD', 'TZS'])
...
/**
 * @swagger
 * /api/stripe/subscriptions/{id}/force-delete:
 *   delete:
 *     summary: Force delete subscription
 *     tags: [Stripe Subscriptions]
 *     security:
 *       - SuperAdminAuth: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Subscription force deleted successfully
 *       404:
 *         description: Subscription not found
 *       500:
 *         description: Server error
 */
router.delete('/subscriptions/:id/force-delete', [
  param('id').isMongoId().withMessage('Invalid subscription ID')
], validateRequest, StripeController.forceDeleteSubscription);
