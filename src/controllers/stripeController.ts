...
export class StripeController {
  ...
  static async createSubscription(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      // Validate plan's Stripe price exists (handles migrated DB with old/different Stripe account)
      try {
        await stripe.prices.retrieve(plan.stripe_price_id);
      } catch (priceError: any) {
        if (priceError?.code === 'resource_missing' || priceError?.raw?.code === 'resource_missing') {
          res.status(400).json({
            success: false,
            message: `Plan "${plan.name}" has an invalid Stripe price ID. The price may have been deleted or belongs to a different Stripe account. Please update the plan or create a new plan.`
          });
          return;
        }
        throw priceError;
      ...
      // Create or retrieve Stripe customer (Stripe requires one currency per customer)
      let stripeCustomer;
      try {
        ...
        let customerWithMatchingCurrency: Stripe.Customer | null = null;
        ...
        for (const c of customers.data) {
          ...
          const hasDifferentCurrency = subscriptions.data.some((sub) => {
            const subCurrency = sub.items?.data?.[0]?.price?.currency;
            return subCurrency && subCurrency !== planCurrency;
          });
          if (!hasDifferentCurrency || subscriptions.data.length === 0) {
            customerWithMatchingCurrency = c;
            break;
          }
        }
        
        if (customerWithMatchingCurrency) {
          stripeCustomer = customerWithMatchingCurrency;
        } else if (customers.data.length > 0) {
          // Existing customer has different currency - create new customer (Stripe allows same email)
      ...
      if (admin_payment_method_id) {
        ...
        try {
          ...
          let errMsg = paymentError.message || 'Payment failed';
          if (errMsg.includes('cannot combine currencies')) {
            errMsg = 'This customer already has subscriptions in a different currency. Please use a plan with the same currency (USD).';
          } else if (paymentError?.code === 'resource_missing' && paymentError?.param?.includes('price')) {
            errMsg = 'The subscription plan has an invalid Stripe price. Please update the plan or create a new one.';
          }
          res.status(400).json({
            success: false,
      ...
      if (message.includes('cannot combine currencies')) {
        message = 'This customer already has subscriptions in a different currency. Please use a plan with the same currency (USD) or contact support.';
      } else if (error?.code === 'resource_missing' && error?.param?.includes('price')) {
        message = `The subscription plan has an invalid Stripe price. Please update the plan or create a new one in your Stripe dashboard.`;
      }
      res.status(500).json({
        success: false,
        message
  ...
  static async cancelSubscription(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        ...
        // If subscription doesn't exist in Stripe (e.g. deleted, wrong account), sync local DB
  ...
  /**
   * Force delete subscription - removes from database without Stripe sync.
   * Use for orphaned records where Stripe subscription no longer exists.
   */
  static async forceDeleteSubscription(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      res.status(500).json({
        success: false,
        message: error.message || 'Failed to force delete subscription'
      });
    }
  }
