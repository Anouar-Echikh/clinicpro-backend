...
import { Patient, Appointment, Prescription, User, UserClinic } from '../models';
...
export class PatientController {
  static async createPatient(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      // Extract password from request (if provided)
      ...
      // If password is provided, create a corresponding User record with 'patient' role
      if (password) {
        try {
          ...
          if (!existingUser) {
            ...
            const userData = {
              ...
              password_hash: hashedPassword,  // Already hashed
            ...
            // Use insertMany to bypass the pre-save hook
            const [createdUser] = await User.insertMany([userData]);

            // Link user to clinic via UserClinic
            const userClinic = {
            ...
            await UserClinic.create(userClinic);
            
            console.log(`Created User account for patient: ${patientData.email}`);
          } else {
            console.log(`User account already exists for email: ${patientData.email}`);
          }
        } catch (userError: any) {
          console.error('Error creating user account for patient:', userError);
          // Don't fail the patient creation if user creation fails
  ...
  static async updatePatient(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      // If password is provided, update the corresponding User's password
      if (password) {
        try {
          // Find the user associated with this patient's email
          ...
          if (user) {
            ...
            // Update user's password directly using updateOne to bypass pre-save hook
            await User.updateOne(
              { _id: user._id },
              { $set: { password_hash: hashedPassword } }
            );
            
            console.log(`Updated password for patient user: ${patient.email}`);
          } else {
            console.log(`No user account found for patient: ${patient.email}. Cannot update password.`);
          }
        } catch (userError: any) {
          console.error('Error updating user password for patient:', userError);
          // Don't fail the patient update if password update fails
          // But log it for admin awareness
        }
      }
